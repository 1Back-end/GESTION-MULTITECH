            ...
            <div class="navbar-nav ms-auto">
                ...
                <div class="nav-item dropdown">
                    ...
                    <div class="d-block d-lg-none">
                        <a href="login/login.php" class="btn-add btn btn-dark text-white outline-none border-0 btn-sm shadow-none btn-lg">Se connecter</a>
                        <span class="mx-2"></span>
                        <a href="utilisateurs/creation_compte.php" class="btn-add btn btn-success outline-none btn-sm text-white px-3 shadow-none btn-lg">S'Inscrire</a>
                    </div>
                     <br><br>
                    <!-- Pour les grands écrans -->
                    <div class="d-none d-lg-block">
                        <div class="d-flex align-items-center justify-content-center">
                              <a href="login/login.php" class="btn btn-dark border-0 rounded-0">Se connecter</a>
                              <span class="mx-2"></span>
                              <a href="utilisateurs/creation_compte.php" class="btn btn-success shadow-none border-0 rounded-0">S'Inscrire</a>
                            </div>
...
<!-- Navbar End -->
        ...
        <div class="container-fluid header bg-white p-0">
            ...
            <div class="col-md-6 p-5 mt-lg-5">
                    ...
                    <a href="#produit" class="btn btn-primary py-3 px-5 me-3 animated fadeIn shadow-none">Nos Produits</a>
                    <!--<a href="Apk/IMMO INVESTMENT.apk" class="btn btn-primary py-3 px-5 me-3 animated fadeIn">Télécharger l'APK</a>-->
                </div>
            ...
            <div class="container">
                <div class="row g-2">
                    ...
                    <form  method="GET">
                        <div class="row g-2">
                            <div class="col-md-3">
                                <select name="region" id="regions" class="form-select border-0 py-3">
                            ...
                            <div class="col-md-3">
                                <select name="ville" id="villes" class="form-select border-0 py-3">
                                    <option disabled selected>Sélectionner une option</option>
                                </select>
                            </div>
                            <div class="col-md-3">
                                    <select name="quartier" id="quartiers" class="form-select border-0 py-3">
                                        <option disabled selected>Sélectionner une option</option>
                                    </select>
                            </div>
                    ...
                    <div class="col-md-2">
                        <button type="submit" name="submit" class="btn btn-dark border-0 w-100 py-3 shadow-none">Rechercher</button>
                    </div>
                </div>
                </form>
            </div>
 ...
 <div class="container-xxl py-5 ">
        ...
        <div class="container-xxl py-5">
            ...
            <ul class="nav justify-content-center mt-4 wow fadeInUp" data-wow-delay="0.2s">
            <li class="nav-item">
                <a class="nav-link btn btn-outline-primary rounded-pill mx-2 px-4 py-2" href="#">Produit VIP</a>
            </li>
            <li class="nav-item">
                <a class="nav-link btn btn-outline-success rounded-pill mx-2 px-4 py-2" href="#">Produit Standard</a>
            </li>
            <li class="nav-item">
                <a class="nav-link btn btn-outline-warning rounded-pill mx-2 px-4 py-2" href="#">Produit Premium</a>
            </li>
            </ul>
 ...
 <?php
// Fonction pour déterminer la catégorie du logement
function getCategorieLogement($type, $prix) {
    if ($type === 'Chambre Moderne') {
        return $prix < 25000 ? 'Standard' : ($prix < 35000 ? 'Premium' : 'VIP');
    } elseif ($type === 'Studio Moderne') {
        return $prix < 50000 ? 'Standard' : ($prix < 75000 ? 'Premium' : 'VIP');
    } elseif ($type === 'Appartement Moderne') {
        return $prix < 70000 ? 'Standard' : ($prix < 150000 ? 'Premium' : 'VIP');
    } elseif ($type === 'Villa') {
        return $prix < 150000 ? 'Standard' : ($prix < 250000 ? 'Premium' : 'VIP');
    } elseif ($type === 'Duplex') {
        return $prix < 280000 ? 'Standard' : ($prix < 350000 ? 'Premium' : 'VIP');
    }
    return null; // Pas de catégorie pour les autres types
}

// Initialisation des filtres
$typePropriete = $_GET['type_propriete'] ?? '';
$ville = $_GET['ville'] ?? '';
$region = $_GET['region'] ?? '';
...
$baseCondition = "statut = 'Accepté' AND STATUS = 'Present'";
$filters = [];
if (!empty($typePropriete)) $filters[] = "type_logement = " . $connexion->quote($typePropriete);
if (!empty($ville)) $filters[] = "ville = " . $connexion->quote($ville);
if (!empty($region)) $filters[] = "region = " . $connexion->quote($region);
if (!empty($quartier)) $filters[] = "quartier = " . $connexion->quote($quartier);
...
// Requête de comptage
$countSql = "SELECT COUNT(*) AS total FROM produits WHERE $whereClause";

// Exécution
$result = $connexion->query($sql);
$totalCountResult = $connexion->query($countSql);
$totalCount = $totalCountResult->fetch(PDO::FETCH_ASSOC)['total'];
$totalPages = ceil($totalCount / $itemsPerPage);

if ($result): ?>
    <div class="row">
        <?php if ($totalCount > 0): foreach ($result as $row): ?>
            <?php
            $categorie = getCategorieLogement($row['type_logement'], $row['prix']);
            $badge = '';

            if ($categorie) {
                $badgeClass = match($categorie) {
                    'Standard' => 'badge bg-secondary border-0 rounded-0',
                    'Premium'  => 'badge bg-warning border-0 rounded-0',
                    'VIP'      => 'badge bg-success border-0 rounded-0',
                    default    => 'badge bg-dark border-0 rounded-0'
                };
                $badge = "<span class='$badgeClass'>$categorie</span>";
            }
            ?>
            <div class="col-lg-4 col-sm-12 mb-4">
                <div class="shadow p-3 bg-white wow fadeInUp rounded-0 border-0 text-center" data-wow-delay="0.5s">
                    <div class="card border-0 h-100">
                        <div class="img-area mb-4">
                            <?php if (!empty($row['photo_principale'])): ?>
                                <img src="uploads/<?php echo $row['photo_principale']; ?>" class="img-card img-thumbnail" style="height:200px;" alt="Image du produit">
                            <?php else: ?>
                                <img src="img.jpg" class="img-card img-thumbnail" alt="Image du produit">
                            <?php endif; ?>
                        </div>

                        <p class="text-center text-truncate" style="max-width: 300px;"><?php echo $row['description']; ?></p>

                        <?php if (!empty($badge)): ?>
                            <p class="text-muted">Catégorie : <?php echo $badge; ?></p>
                        <?php endif; ?>
                    </div>
                    <div class="pb-2" style="position: relative; bottom: 5px;">
                        <a href="page/details.php?id=<?php echo $row['id']; ?>" class="btn btn-sm btn-add border-0 rounded-0 p-2 btn-success text-white mx-2 shadow-none">
        ...
        <?php endforeach; else: ?>
            <div class="alert alert-danger" role="alert">Aucun résultat trouvé.</div>
        <?php endif; ?>
    </div>
        ...
        <div class="col-md-12 mt-3 col-sm-12 text-center">
            <ul class="pagination justify-content-center">
                <?php for ($i = 1; $i <= $totalPages; $i++): ?>
                    <li class="page-item shadow-none <?php echo ($i == $currentPage) ? 'active' : ''; ?>">
                        <a class="page-link border-0 rounded-0" href="?page=<?php echo $i; ?>"><?php echo $i; ?></a>
                    </li>
                <?php endfor; ?>
            </ul>
        </div>
...
<?php else: ?>
    <div class="alert alert-danger" role="alert">Erreur de requête : <?php echo $connexion->errorInfo()[2]; ?></div>
<?php endif; ?>
