...
$reservations = get_reservation_nuitee_by_motel_id($connexion, $motel_id, $perPage, $offset);

// Total des réservations pour la pagination
$totalReservations = get_total_reservation_nuitee($connexion, $motel_id);
   ...
   <div class="col-md-12 col-sm-12 mb-3">
       <div class="card-box p-3">
            <div class="text-center">
                <h5 class="text-uppercase">Liste des nuitées du motel : <?php echo $motel_name; ?></h5>
            </div>
        </div>
        </div>
                            ...
                            <?php if (count($reservations) > 0): ?>
                                <?php foreach ($reservations as $index => $reservation): ?>
                                    <tr>
                                        ...
                                        <td><?= htmlspecialchars((new DateTime($reservation['date_entre']))->format('d-m-Y H:i')); ?></td>
                                        <td><?= htmlspecialchars((new DateTime($reservation['date_sortie']))->format('d-m-Y H:i')); ?></td>
                                        <td><?= htmlspecialchars($reservation['client_first_name']) . ' ' . htmlspecialchars($reservation['client_last_name']); ?></td>
                                        <td><?= htmlspecialchars((new DateTime($reservation['created_at']))->format('d-m-Y H:i')); ?></td>
